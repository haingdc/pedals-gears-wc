...
const UtilsWindowArray = {
  getWindowArray() {
    return JSON.parse(localStorage.getItem(LOCAL_STORAGE.window_manager) || "[]");
  },
  setWindowArray(windowArray: WindowInfo[]) {
    localStorage.setItem(LOCAL_STORAGE.window_manager, JSON.stringify(windowArray));
  },
  dropWindowArray() {
...
class WindowStateManager {
  ...
  #removeWindow() {
    ...
    this.#windowArray = this.#windowArray.filter((window) => window.id !== this.#windowId);
  ...
  #startHeartbeat(): void {
    ...
    const now = Date.now();
    const index = this.#windowArray.findIndex((window) => window.id === this.#windowId);
    if (index !== -1) {
    ...
    this.#windowArray = this.#windowArray.filter((window) => now - window.heartbeat < 5000);
  ...
  public removeEventListener(
    ...
    if (eventName === WINDOW_UPDATE_EVENT) {
      ...
      if (this.#listeners.length === 0) {
        ...
        // if (UtilsWindowArray.getWindowArray().length === 1) {
